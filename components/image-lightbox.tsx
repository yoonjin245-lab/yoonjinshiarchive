...
export function ImageLightbox({ item, onClose }: ImageLightboxProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex min-h-[45vh] items-center justify-center bg-neutral-100 p-5 sm:min-h-[72vh]">
          <img src={item.imageUrl} alt={item.alt ?? item.caption ?? item.id} className="max-h-full max-w-full object-contain" />
        </div>
        <aside className="relative flex flex-col gap-7 border-t border-neutral-200 p-5 text-sm font-normal sm:border-l sm:border-t-0 sm:p-6">
          <div className="flex items-start justify-between gap-6">
            ...
            <button type="button" onClick={onClose} className="text-xl leading-none text-neutral-500 hover:text-black" aria-label="Close image popup">
              X
            </button>
          </div>
          ...
          {item.description ? <p className="max-w-prose text-[15px] leading-relaxed text-neutral-700">{item.description}</p> : null}
